  ...
  return (
    <div className="min-h-full flex flex-col items-center justify-center p-6">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <h1 className="text-xl font-semibold text-center mb-1">{t('auth.appTitle')}</h1>
        <p className="text-sm text-slate-500 text-center mb-6">{t('auth.appSubtitle')}</p>

        <button
          onClick={handleGoogle}
          disabled={busy}
          className="w-full flex items-center justify-center gap-2 bg-teal-600 text-white rounded-lg py-2.5 px-4 text-sm font-medium disabled:opacity-50"
        >
        ...
        <div className="relative my-5">
          ...
          <div className="relative flex justify-center text-xs text-slate-400"><span className="bg-white px-2">{t('common.or')}</span></div>
        </div>
